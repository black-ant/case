            ...
            try {
                if (field.getAnnotation(ADAnnotation.class) != null && null != field.get(baseBean)) {
                    if ("memberOf".equals(field.getAnnotation(ADAnnotation.class).alias()) || "ldapGroups".equals(field.getAnnotation(ADAnnotation.class).alias())) {
                        ...
                        attributes.add(AttributeBuilder.build(field.getAnnotation(ADAnnotation.class).alias(), memberOf));
                    } else {
                        attributes.add(AttributeBuilder.build(field.getAnnotation(ADAnnotation.class).alias(), field.get(baseBean)));
                    }
