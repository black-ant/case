...
| 配置项 | 环境变量 | 默认值 | 说明 |
|--------|---------|--------|------|
| **API Key** | `OPENAI_API_KEY` | - | OpenAI API 密钥（必填） |
| **Base URL** | `OPENAI_BASE_URL` | `https://api.openai.com` | API 端点（支持代理） |
| Model | `OPENAI_MODEL` | `gpt-3.5-turbo` | 使用的模型 |
| Temperature | `OPENAI_TEMPERATURE` | `0.7` | 温度参数 |
| Server Port | `SERVER_PORT` | `8080` | 服务器端口 |

📖 **详细配置文档**：[README-CONFIG.md](./README-CONFIG.md)

### 使用代理或自定义端点

如果你使用代理或自定义的 OpenAI 兼容端点：
