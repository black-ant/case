...
public class CustomerServiceAgent {
    ...
    public String chat(String sessionId, String customerId, String userMessage) {
        ...
        // 使用动态 ChatClient 调用 AI（带重试机制）
        String assistantMessage = dynamicChatClientService.chatWithRetry(sessionId, userMessage, chatClient -> {
            ChatResponse response = chatClient.prompt()
                .messages(messages)
                .function("orderQueryFunction", "查询订单信息", orderQueryFunction)
                .function("trackingQueryFunction", "查询物流追踪信息", trackingQueryFunction)
                .function("ticketCreateFunction", "创建客服工单", ticketCreateFunction)
                .function("ticketQueryFunction", "查询工单列表", ticketQueryFunction)
                .call()
                .chatResponse();
    ...
    public Flux<String> chatStream(String sessionId, String customerId, String userMessage) {
        ...
        // 使用动态 ChatClient 调用 AI（流式，带重试机制）
        return dynamicChatClientService.chatStreamWithRetry(sessionId, userMessage, chatClient -> {
